...
// Drives the P8 panel directly to verify HUB75E wiring and five-address scanning.
static void p8FiveScanRawTask(void *) {
  constexpr uint8_t dataPins[] = {42, 41, 40, 38, 39, 37};
  constexpr uint8_t addressPins[] = {45, 36, 48};
  constexpr uint8_t latchPin = 47;
  constexpr uint8_t outputEnablePin = 14;
  constexpr uint8_t clockPin = 2;

  for (uint8_t pin : dataPins) pinMode(pin, OUTPUT);
  for (uint8_t pin : addressPins) pinMode(pin, OUTPUT);
  ...
  for (;;) {
    ...
    for (uint8_t row = 0; row < 5; row++) {
      ...
      digitalWrite(addressPins[1], row & 0x02);
...
void WLED::setup()
{
  ...
  Serial.begin(115200);
#ifdef WLED_P8_5S_DIAGNOSTIC
  xTaskCreatePinnedToCore(p8FiveScanRawTask, "p8-5s-raw", 2048, nullptr, 2, nullptr, 0);
#endif
