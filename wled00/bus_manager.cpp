...
// AI: below section was generated by an AI
// Maps a logical P8 40x20 coordinate onto the initial 80x10 DMA hypothesis.
void BusHub75Matrix::drawFiveScanPixel(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b) {
  ...
  const int16_t scanRows = _physicalPanelHeight / 4;
  const int16_t halfHeight = _physicalPanelHeight / 2;
  const int16_t rawX = x + ((y % halfHeight) < scanRows ? _physicalPanelWidth : 0);
  const int16_t rawY = (y / halfHeight) * scanRows + (y % scanRows);
