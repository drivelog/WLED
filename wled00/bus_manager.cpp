...
BusHub75Matrix::BusHub75Matrix(const BusConfig &bc) : Bus(bc.type, bc.start, bc.autoWhite) {
  ...
  _physicalPanelWidth = physicalPanelWidth;
  _physicalPanelHeight = physicalPanelHeight;
  ...
  if (bc.type == TYPE_HUB75MATRIX_HS) {
  ...
  } else if (bc.type == TYPE_HUB75MATRIX_5S) {
      // AI: below section was generated by an AI
      // Each chained 40x20 1:5 panel shifts 80 columns for each of its 10 DMA rows.
      if (physicalPanelWidth != 40 || physicalPanelHeight != 20 || chainLength < 1 || chainLength > 4 || _rows != 1 || _cols != chainLength) {
        DEBUGBUS_PRINTLN(F("HUB75 1:5 requires one horizontal row of up to four 40x20 panels"));
        return;
      }
      _isFiveScan = true;
      ...
      // Chained panels need contiguous DMA-capable internal RAM; S3 PSRAM cannot
      // be relied upon for these buffers. Four-bit PWM keeps up to four panels
      // within the available DMA heap while retaining full RGB input colors.
      if (chainLength > 1) mxconfig.setPixelColorDepthBits(4);
      // AI: end
  } else {
...
*/
  ...
  */
  ...
  if (_valid) {
    ...
    if (_isFiveScan) _panelWidth = physicalPanelWidth * mxconfig.chain_length;
...
void IRAM_ATTR BusHub75Matrix::setPixelColor(unsigned pix, uint32_t c) {
  ...
  if (_ledBuffer) {
    ...
    if (_isFiveScan) {
      ...
      drawFiveScanPixel(int16_t(x), int16_t(y), r, g, b);
    } else if (virtualDisp != nullptr) {
...
void BusHub75Matrix::show(void) {
  ...
  if (_ledBuffer) {
    ...
    unsigned height = _isFiveScan ? _physicalPanelHeight : (_isVirtual ? virtualDisp->height() : display->height());
    ...
    for (int y=0; y<height; y++) for (int x=0; x<width; x++) {
      if (getBitFromArray(_ledsDirty, pix) == true) {        // only repaint the "dirty"  pixels
        ...
        if (_isFiveScan) drawFiveScanPixel(int16_t(x), int16_t(y), c.r, c.g, c.b);
        else if (_isVirtual) virtualDisp->drawPixelRGB888(int16_t(x), int16_t(y), c.r, c.g, c.b);
...
// AI: below section was generated by an AI
// Maps a logical P8 40x20 coordinate onto its measured 80x10 DMA layout.
void BusHub75Matrix::drawFiveScanPixel(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b) {
  if (x < 0 || x >= _physicalPanelWidth * mxconfig.chain_length || y < 0 || y >= _physicalPanelHeight) return;
  const uint8_t panel = x / _physicalPanelWidth;
  const uint8_t localX = x % _physicalPanelWidth;
  const uint8_t driverPair = localX / 8;
  const uint8_t offset = localX % 8;
  const bool secondBank = (y % 10) >= 5;
  const int16_t rawX = panel * _physicalPanelWidth * 2 + driverPair * 16 + (secondBank ? 8 + offset : 7 - offset);
  const int16_t rawY = (y >= 10 ? 5 : 0) + (y % 5);
  display->drawPixelRGB888(rawX, rawY, r, g, b);
}

#ifdef WLED_P8_5S_DIAGNOSTIC
// Draws alternating row, column, and grid patterns without requiring WLED setup.
void BusHub75Matrix::drawFiveScanDiagnostic() {
  ...
  for (int16_t y = 0; y < _physicalPanelHeight; y++) {
    for (int16_t x = 0; x < _physicalPanelWidth; x++) {
      uint8_t r = 0, g = 0, b = 0;
      if (page == 0) {
        const uint8_t *color = colors[y % 5];
        r = color[0]; g = color[1]; b = color[2];
        if (x == 0 || x == y * 2) r = g = b = 255;
      } else if (page == 1) {
        const uint8_t *color = colors[(x / 8) % 5];
        r = color[0]; g = color[1]; b = color[2];
        if (y == 0 || y == 19) r = g = b = 255;
      } else {
        const bool line = x == 0 || x == 39 || y == 0 || y == 19 || x % 10 == 0 || y % 5 == 0;
        if (line) r = g = b = 255;
        else if (((x / 5) + (y / 5)) & 1) b = 96;
      }
      drawFiveScanPixel(x, y, r, g, b);
    }
  }
}
#endif
...
std::vector<LEDType> BusHub75Matrix::getLEDTypes() {
  return {
    ...
    {TYPE_HUB75MATRIX_5S,     "H",     PSTR("HUB75 P8 40x20 (1:5 Scan)")},
  };
}

size_t BusHub75Matrix::getPins(uint8_t* pinArray) const {
  if (pinArray) {
    pinArray[0] = (_isQuadScan || _isFiveScan) ? _physicalPanelWidth : mxconfig.mx_width;
    pinArray[1] = (_isQuadScan || _isFiveScan) ? _physicalPanelHeight : mxconfig.mx_height;
