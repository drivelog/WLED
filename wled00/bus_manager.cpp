...
BusHub75Matrix::BusHub75Matrix(const BusConfig &bc) : Bus(bc.type, bc.start, bc.autoWhite) {
  ...
  if (bc.type == TYPE_HUB75MATRIX_HS) {
      ...
      // A 40x20 1:5 panel shifts 80 columns for each of its 10 DMA rows.
      if (physicalPanelWidth != 40 || physicalPanelHeight != 20 || chainLength != 1 || _rows != 1 || _cols != 1) {
        DEBUGBUS_PRINTLN(F("HUB75 1:5 diagnostic currently requires one 40x20 panel"));
        return;
      }
      _isFiveScan = true;
      mxconfig.chain_length = 1;
...
void BusHub75Matrix::drawFiveScanPixel(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b) {
  if (x < 0 || x >= _physicalPanelWidth || y < 0 || y >= _physicalPanelHeight) return;
  const uint8_t driverPair = x / 8;
  const uint8_t offset = x % 8;
  const bool secondBank = (y % 10) >= 5;
  const int16_t rawX = driverPair * 16 + (secondBank ? 8 + offset : 7 - offset);
